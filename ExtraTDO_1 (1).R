...
df1<-read_csv("/Users/Daniele/Desktop/tdo/dataset/sumstats_protests.csv") #Perché dà l'errore? 
df1<-read_csv2("/Users/Daniele/Desktop/tdo/dataset/sumstats_protests.csv") #Dobbiamo sempre controllare che abbiamo indicato il passaggio/nome corretto
...
#####Errori comuni####
df2 <- read_csv("/Users/Daniele/Desktop/tdo/dataset/IT_snap_election.csv") #dobbiamo sempre specificare l'estensione del file, csv nel nostro caso
...
remove(df1)   #per togliere oggetti che non usiamo dopo 
########################################################################
...
df_prova=read_csv("/Users/Daniele/Desktop/tdo/dataset/IT_snap_election.csv",skip = 17) # Cosa è cambiato rispetto al df2? Vengono saltate le prime 17 righe
remove(df_prova)
########################################################################
...
# Valore medio = 751.9, Io in realtà le vedo, anche se Post.created.date dovrebbe essere di tipo data. e message di tipo character. Massimo angry = 3106.00 e minimo di shares = 4.0

install.packages("inspectdf")
library(inspectdf)
library(readr)
inspect_na(df2) %>%
  print(n=Inf)#vediamo se abbiamo i valori mancanti. Quali variabili hanno i valori mancanti e quanti osservazioni ci mancano? 
# Vediamo che in message ci sono 6 valori mancanti, che rappresentano 1.97% di valori mancanti sul totale, e Link ha 1 valore mancante, uguale allo 0.328% dei valori mancanti. 
...
df2 %>% 
  ...
  mutate(positive = Wow + Haha, #creiamo la colonna di somma di reazioni positive,
         ...
         negative_share = (negative/(Love+Wow+Haha+Sad+Angry+Care)/negative)*100 ) %>% 

#percentuale dei emoji negativi a reazioni totali
